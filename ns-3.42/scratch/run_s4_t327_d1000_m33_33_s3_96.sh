...
# Script d'automatisation pour les simulations LoRaWAN ADR - Scénario 4
# Configuration: T=327, D=1000, M=33.33, S=3.96
# Fichier: run_s4_t327_d1000_m33_33_s3_96.sh
...
echo -e "${GREEN}  T=327, D=1000, M=33.33, S=3.96${NC}"
echo -e "${GREEN}========================================${NC}"
            ...
            for sigma in "${SIGMAS_S4[@]}"; do
                for run in $(seq 1 $NUM_RUNS); do
                    for adr_algo in "${ADR_ALGOS[@]}"; do
                        ...
                        echo -e "${GREEN}[S4: $progress%] Running: Traffic=$traffic s, Density=$density, Mobility=$mobility km/h, Sigma=$sigma, ADR=$adr_algo, Run=$run/$NUM_RUNS${NC}"
